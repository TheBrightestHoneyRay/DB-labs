...
SELECT SUM(budget) AS bud_sum FROM departments;
...
SELECT name AS bud_cost FROM departments
ORDER BY budget DESC;

SELECT DISTINCT ON(budget) name
FROM departments
ORDER BY budget DESC
LIMIT 1 offset 1;
...
WHERE department = (SELECT code FROM departments WHERE employees.name = 'Research');

DELETE FROM employees AS e
    USING departments
      AS d where d.name = 'IT' AND e.department = d.code;
